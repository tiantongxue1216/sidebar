        ...
        <template v-for="(item, index) in menuList">
            <div style="text-align: center;" :key="index">
                <Dropdown transfer v-if="item.children && item.children.length > 1" placement="right-start" :key="index" @on-click="changeMenu">
                    <Button style="width: 50px;margin-left: -5px;padding:10px 0;" type="text">
                ...
                <Dropdown transfer v-else-if="item.children.length === 0" placement="right-start" :key="index" @on-click="changeMenu(item.name)">
                    <Button style="width: 50px;margin-left: -5px;padding:10px 0;" type="text">
                        <Icon :type="item.icon" size="18"></Icon>
                    </Button>
                    <DropdownMenu style="width: 150px;" slot="list">
                        <template>
                            <DropdownItem :name="item.name" :key="item.name">
                ...
                <Dropdown transfer v-else placement="right-start" :key="index" @on-click="changeMenu">
                    <Button @click="changeMenu(item.children[0].name)" style="width: 50px;margin-left: -5px;padding:10px 0;" type="text">
                        <Icon :type="item.icon" size="18"></Icon>
                    </Button>
